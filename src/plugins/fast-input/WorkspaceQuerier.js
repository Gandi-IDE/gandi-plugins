...
import { BlockInputType, BlockInstance, BlockShape, BlockInputEnum, BlockInputString } from "./BlockTypeInfo.js";

import { hz2py_dict } from "./pinyin_dict_notone.js";
  ...
  *parseTokens(query, idx, depth) {
    for (let valueIdx = 0; valueIdx < this.values.length; valueIdx++) {
      ...
      if (remainingChar < valueInfo.lower.length) {
        ...
        } else if (valueInfo.abbrs && valueInfo.abbrs.find((abbr) => abbr.startsWith(substr))) {
          const end = remainingChar < 0 ? 0 : query.length;
          yield new Token(idx, end, this, valueInfo, { isTruncated: true, isDefiningFeature });
        }
      } else {
        ...
        } else if (valueInfo.abbrs && valueInfo.abbrs.find((abbr) => query.lowercase.startsWith(abbr, idx))) {
          yield new Token(idx, idx + valueInfo.lower.length, this, valueInfo, { isDefiningFeature });
        }
      }
    }
...
function _flatPolyphone(array, splitter, joinChar) {
  splitter = splitter || "";
  joinChar = joinChar || "";
  var result = [""],
    temp = [];
...
/**
 * Converts a string to its abbreviation using a Chinese character to Pinyin dictionary.
 * If the string contains ASCII characters only, it returns undefined.
 * If the Pinyin dictionary is not initialized, it initializes it before converting.
 * @param {string} str - The input string to be converted.
 * @returns {string[]|undefined} - The abbreviation of the input string, or undefined if the input string contains non-Chinese characters.
 */
function _hz2abbr(str) {
  if (str.split("").every((c) => c.charCodeAt(0) < 128)) {
    return undefined;
  }
  let abbrs = str.split("").map((c) => {
    const py = hz2py_dict[c];
    if (py === undefined) {
      return c;
    }
    if (Array.isArray(py)) {
      ...
      while (true) {
        ...
        if (wordEnd === i) {
          ...
          for (let formPartIdx = lastPartIdx + 1; formPartIdx < stringForm.strings.length; formPartIdx++) {
            ...
            const stringFormAbbr = stringForm.abbrs[formPartIdx];
            if (stringFormAbbr && stringFormAbbr.find((abbr) => abbr.startsWith(word))) {
              match = formPartIdx;
              break;
            }
          }
