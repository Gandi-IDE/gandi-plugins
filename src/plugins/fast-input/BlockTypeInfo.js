...
export class BlockInputEnum extends BlockInput {
  ...
  constructor(options, inputIdx, fieldIdx, isRound) {
    ...
    for (let i = 0; i < options.length; i++) {
      // compatible with the case where the option is a number or boolean
      if (typeof options[i][1] === "number" || typeof options[i][1] === "boolean") {
        options[i][1] = String(options[i][1]);
      }
      ...
      if (field.className_ === "blocklyText blocklyDropdownText") {
        ...
        switch (field.src_.split("/").pop()) {
          ...
          case "rotate-right.svg":
            parts.push(locale("general.blocks.clockwise"));
            break;
          case "rotate-left.svg":
            parts.push(locale("general.blocks.anticlockwise"));
            break;
        }
      } else {
        if (!field.argType_) {
